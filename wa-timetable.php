		...
		if (!empty($day_sessions)) {
			...
			foreach ($day_sessions as $session_name => $session_events) {
				if ($session_name !== '') { // Only display if the session name is not empty
					...
					$output .= '<h2 class="accordion-header pb-0 my-0" id="' . $session_heading_id . '">';
...
function wa_timetable_enqueue_scripts() {
	...
	wp_enqueue_style('wa-timetable-styles', plugin_dir_url(__FILE__) . 'css/wa-timetable-styles.css', array('bootstrap'), '1.0.3');
